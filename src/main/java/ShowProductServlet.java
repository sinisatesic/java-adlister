// This Servlet will be used to listen for users trying to view all of the Products - likely by going to a page like "/products" ("/products/index.jsp")
...
public class ShowProductServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        // check to see if there is an id passed in URL parameters
            // i.e. http://localhost:8080/products/show?id=27 (hypothetical 27th item)
        ...
        // set the attribute "product" to the object we just created ^ (line 26)
        request.setAttribute("product", product);
        request.getRequestDispatcher("/product/product-show.jsp").forward(request, response);
    }
}
